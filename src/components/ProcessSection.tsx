...
const ProcessSection = () => (
  <>
    <section id="process" className="relative pt-[120px] pb-16 px-5 md:pb-32 md:px-10" style={{ background: "#171410", borderTop: "1px solid rgba(196,153,58,0.25)" }}>
      <div className="max-w-[1200px] mx-auto">
        <div className="reveal-section mb-8 md:mb-12">
          <span className="font-mono text-[11px] md:text-[12px] text-gold tracking-[0.2em] uppercase">How We Work</span>
        </div>

        <div className="reveal-section grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 mb-16 md:mb-24">
          <h2 className="font-serif text-[clamp(32px,8vw,56px)] md:text-[clamp(32px,4.5vw,56px)] font-light leading-[1.1]">
            A process<br />built for<br /><em className="text-gold">speed.</em>
          </h2>
          <p className="font-mono text-[16px] md:text-[16px] text-body-text leading-[1.75] md:leading-[1.75] self-end">
        ...
        <div className="reveal-section grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
          {steps.map((step, i) => (
            ...
            >
              <span className="font-mono text-[11px] md:text-[12px] text-label-text">{step.num}</span>
              <h3 className="font-serif text-[22px] md:text-[24px] font-light text-foreground mt-3 md:mt-4 mb-4 md:mb-5">{step.title}</h3>
              <p className="font-mono text-[14px] md:text-[14px] text-body-text leading-[1.75] md:leading-[1.75] mb-6 md:mb-8">{step.desc}</p>
              <span className={"font-mono text-[11px] md:text-[12px] text-gold"}>{step.meta}</span>
            </div>
          ))}
        </div>
      </div>
