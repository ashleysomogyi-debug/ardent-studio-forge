...
const ProcessSection = () => (
  <section id="process" className="bg-bg-base py-24 px-6 md:py-32 md:px-10">
    <div className="max-w-[1200px] mx-auto">
      <div className="reveal-section mb-12">
        <span className="font-mono text-[16px] text-gold tracking-[0.2em] uppercase">How we work</span>
      </div>

      {/* Intro */}
      <div className="reveal-section grid grid-cols-1 md:grid-cols-2 gap-16 mb-24">
        <h2 className="font-serif text-[clamp(36px,5vw,56px)] font-light leading-[1.1]">
          A process<br />built for<br /><em className="text-gold">speed.</em>
        </h2>
        <p className="font-mono text-[17px] text-body-text leading-[1.8] self-end">
      ...
      <div className="reveal-section grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
        {steps.map((step, i) => (
          <div
            key={step.num}
            className={`border border-border p-[56px_40px] ${i > 0 ? "lg:border-l-0" : ""} ${i > 1 ? "md:border-t-0 lg:border-t" : ""} ${i === 1 ? "md:border-l-0" : ""} ${i === 3 ? "md:border-l-0" : ""}`}
          >
            <span className="font-mono text-[16px] text-label-text">{step.num}</span>
            <h3 className="font-serif text-[24px] font-light text-foreground mt-4 mb-5">{step.title}</h3>
            <p className="font-mono text-[17px] text-body-text leading-[1.8] mb-8">{step.desc}</p>
            <span className="font-mono text-[16px] text-gold">{step.meta}</span>
          </div>
        ))}
      </div>
    </div>
