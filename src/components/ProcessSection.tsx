  ...
  <section id="process" className="bg-bg-base py-20 px-6 md:py-24 md:px-10">
    <div className="max-w-[1200px] mx-auto">
      <div className="reveal-section mb-16">
        <span className="font-mono text-[14px] text-gold tracking-[0.2em] uppercase">How we work</span>
      </div>
      ...
      <div className="reveal-section grid grid-cols-1 md:grid-cols-2 gap-12 mb-20">
        ...
        <p className="font-mono text-[15px] text-body-text leading-relaxed self-end">
      ...
      <div className="reveal-section grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
        {steps.map((step, i) => (
          ...
          >
            <span className="font-mono text-[14px] text-label-text">{step.num}</span>
            <h3 className="font-serif text-[24px] font-light text-foreground mt-3 mb-4">{step.title}</h3>
            <p className="font-mono text-[15px] text-body-text leading-relaxed mb-6">{step.desc}</p>
            <span className="font-mono text-[14px] text-gold">{step.meta}</span>
          </div>
        ))}
      </div>
    </div>
