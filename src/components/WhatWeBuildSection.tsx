...
const WhatWeBuildSection = () => (
  <section id="services" className="bg-bg-base py-24 px-6 md:py-32 md:px-10">
    <div className="max-w-[1200px] mx-auto">
      <div className="reveal-section mb-12">
        <span className="font-mono text-[16px] text-gold tracking-[0.2em] uppercase">What we build</span>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 reveal-section">
        {cards.map((card, i) => (
          <div
            key={card.num}
            className={`group bg-bg-elevated p-[56px_44px] border border-border hover:bg-bg-hover transition-colors relative overflow-hidden ${i > 0 ? "md:border-l-0" : ""}`}
          >
            ...
            <span className="font-mono text-[16px] text-label-text">{card.num}</span>
            <h3 className="font-serif text-[28px] font-light text-foreground mt-4 mb-5">{card.title}</h3>
            <p className="font-mono text-[16px] text-body-text leading-[1.8] mb-10">{card.desc}</p>
            <span className="font-mono text-[16px] text-gold">{card.price}</span>
          </div>
        ))}
      </div>
    </div>
