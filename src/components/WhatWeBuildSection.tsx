...
const WhatWeBuildSection = () => (
  <section id="services" className="bg-bg-base py-24 px-6 md:px-10">
    <div className="max-w-[1200px] mx-auto">
      <div className="reveal-section mb-16">
        <span className="font-mono text-[10px] text-gold tracking-[0.2em] uppercase">What we build</span>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 reveal-section">
        {cards.map((card, i) => (
          ...
          >
            ...
            <span className="font-mono text-[10px] text-label-text">{card.num}</span>
            <h3 className="font-serif text-[28px] font-light text-foreground mt-3 mb-4">{card.title}</h3>
            <p className="font-mono text-[12px] text-body-text leading-relaxed mb-8">{card.desc}</p>
            <span className="font-mono text-[12px] text-gold">{card.price}</span>
          </div>
        ))}
      </div>
    </div>
